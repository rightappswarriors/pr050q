...
class Liquidations_model extends CI_Model {
	...
	public function view_list($project){
		...
		return $this->db->query("select a.*,b.projectname from $this->tablename a left join projects b on b.id = a.project where $projectsearch a.status = 1 and a.deleted = 'no' and a.dpno = '' order by a.transactdate desc,a.id desc $thislimit");
	}
	
	public function view_info($id){
		return $this->db->query("select b.projectname,a.* from ".$this->tablename." a left join projects b on b.id = a.project where a.id = $id");
    ...
    public function get_acounting_summary($code){
		return $this->db->query("select p.projectname, c.title,d.titles as titlename, count(c.title) as count_trans,sum(c.debit)as total_sum from liquidations a
