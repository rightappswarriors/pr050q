...
class Payables extends CI_Controller {
	...
	{
		$data['page_title']="Payables (Invoices)";
		$data['records'] = $this->payables_model->view_list();
		$data['suppliers'] = $this->companies_model->view_list();
        $data['invoices'] = $this->payables_model->view_invoices();
	...
	public function addnew()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$id = $this->payables_model->insert($data);
			...
			foreach($this->input->post("itemid") as $ind=>$item){
				...
				$this->payables_model->insert_detail($data_detail);
			}
			sleep(1);
			...
			$datalog = array(
				...
				'description' => $this->session->userdata('pms_username')." added new Payable entry.",
				'dateadded' => date("Y-m-d H:i:s")
			);
	...
	public function update_info()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			// UPDATE DETAILS...
			$this->payables_model->delete_detail($id); // delete the old one...
			...
			$datalog = array(
				'user' => $this->session->userdata('pms_userid'),
				'description' => $this->session->userdata('pms_username')." updated Payable info.",
				'dateadded' => date("Y-m-d H:i:s")
			);
			$this->historylog_model->insert($datalog);
	...
	public function editinfo(){
		...
		$data['info']=$this->payables_model->view_info($id);
		...
		$this->load->view('payables_edit',$data);	
	}
