...
$(function () { 
	...
	$(".table").on("click",".btnedit",function(){
		...
		$(".cardedit").show(500);
		$(".cardedit").load("<?=site_url("dailytimerecord/editinfo")?>",{id:rowid});
		return false;
	});
	...
	$(".table").on("click",".btndel",function(){
		...
		$.confirm({
			...
			buttons: {
				confirm: function () {
					window.location.href = "<?=site_url("dailytimerecord/remove/")?>"+rowid;
				},
				cancel: function () {
					//$.alert('Canceled!');
				}
        ...
        <div class="row">
          <div class="col-12">
            <div class="card cardlist">
				  ...
				  <?php
                    ...
                    if($records->num_rows()>0){
                        foreach($records->result() as $row){
                            ...
                            echo "<td>".date("m/d/Y",strtotime($row->fromdate))." &rarr; ".date("m/d/Y",strtotime($row->todate))."</td>";
                            echo "<td>".$row->incharge."</td>";
                            
                            ?>
                      <td class="text-center">
						  <a class="btn btn-info btn-sm <?=$this->session->userdata('pms_editdel')?>" rel="<?=$row->id?>" href="<?=site_url('dailytimerecord/dtreditinfo/'.$row->id)?>">
			  ...
			  <?php $this->load->view("dailytimerecord_addnew") ?>
            </div>
