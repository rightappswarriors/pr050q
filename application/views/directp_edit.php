...
function total_now_edit(){
    ...
    $('#itemcontainer_accounting_edit').find('.txtdefaultdebitedit').val(tamount-(tdisc+whold));
    $('#itemcontainer_accounting_edit').find('.txtdefaultcreditedit').val(tamount-(tdisc+whold));
  ...
  <div class="card-body">
	...
	<div class="form-group">
	  ...
	  <option value="" selected disabled>Select one</option>
		...
		?>
	  </select><?php if(!$row->supplier): ?><code><?=$row->payee?></code> <?php endif; ?>
	</div>
  </div><div class="col-md-4">
	<div class="form-group">
...
<thead class='text-center'>
 <th width="2%">Exc</th>
  ...
  <tbody>
      <?php 
    foreach($details->result() as $ind=>$detail){
    ?>
      <tr><td class="p-2"><input type="checkbox" name="excludes[<?=($ind)?>]" class="form-control" <?=($detail->exclude==1?'checked':'')?> value="1"></td>
