...
$(function () { 
	...
	$(".table").on("click",".btnedit",function(){
		...
		$(".cardedit").show(500);
		//alert(rowid);
		$(".cardedit").load("<?=site_url("purchase/editinfo")?>",{id:rowid});
		return false;
	});
	...
	$(".table").on("click",".btndel",function(){
		...
		$.confirm({
			...
			buttons: {
				confirm: function () {
					window.location.href = "<?=site_url("purchase/remove/")?>"+rowid;
				},
				cancel: function () {
					//$.alert('Canceled!');
				}
          ...
          <div class="col-12">
                ...
                <table id="example2" class="table table-bordered table-hover">
				  ...
				  if($records->num_rows()>0){
					  ...
					  foreach($records->result() as $row){
						  ...
						  <tr>
						  <td><?=$ctr++?></td>
						  <td><?=date("m/d/Y",strtotime($row->podate))?></td>
						  <td><?=$row->pono?></td>
						  <td><?=$row->drno?></td>
						  <td><?=$row->rsno?></td>
						  ...
						  <a class="btn btn-success btn-sm" target="_blank" href="<?=site_url("purchase/print/".$row->id)?>">
            ...
            <!-- /.card -->
			 <div class="card cardnew card-warning" style="display:none;">
			  ...
			  $this->load->view("purchaseorder_addnew",$data);
			  ?>
            </div>
