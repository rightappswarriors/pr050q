<table id="tblsearch<?=$divedit?>" class="table table-bordered table-hover mt-2">
  ...
  <tr>
	...
	<th width="15%" class="p-0 text-center">Stocks</th>
	<th width="15%" class="p-0 text-center">Price</th>
	<th width="10%" class="p-0 text-center"></th>
  </tr>
  </thead>
  <tbody>
  <?php
  if($records->num_rows()>0){
	  foreach($records->result() as $row){
		  ...
		  <td class="itemdescription"><?=$row->itemdescr?></td>
		  <td class="itemunit"><?=$row->itemunit?></td>
		  <td class="itemstocks text-center"><?=$row->stocks?></td>
		  <td class="itemprice text-center"><?=$this->CI->latest_price($row->id,$row->location,$strdate)?></td>
		  <td class="text-center"><a class="btn btn-flat btn-warning btn-sm btn-block btnadditemsearch" rel="<?=$row->id?>" href="#"><i class="fa fa-arrow-down"></i></a></td></tr>
		  <?php 
	  }
  }
  ?>
...
$(function(){
	...
	$("#tblsearch<?=$divedit?>").on("click",".btnadditemsearch",function(){
		...
		var itemstocks = $(this).closest('tr').find('.itemstocks').text();
		var itemprice = $(this).closest('tr').find('.itemprice').text();
		
        itemprice = itemprice.replace(",","");
        itemprice = itemprice.replace(",","");
        itemprice = itemprice.replace(",","");
        
		if( str_arr.search(txtsearch)<0 && parseFloat(itemstocks)>0 ){
			
			window.globalitems_arr[window.globalCtr] = itemid;
			
			// ADD ITEMS...
			$("#itemscontainer<?=$divedit?>").prepend("<tr><td><input type='number' step='0.01' name='itemstocks[]' value='"+itemstocks+"' class='form-control form-control-sm txtstocks text-right' disabled></td><td><input type='hidden' name='itemid[]' value='"+itemid+"'><input type='number' step='0.01' name='itemqty[]' value='1' min='0.01' max='"+itemstocks+"' class='form-control form-control-sm txtqty'></td><td>"+itemunit+"</td><td>"+itemdescr+"</td><td><input type='number' step='0.01' name='itemprice[]' value='"+itemprice+"' class='form-control form-control-sm txtprice'></td><td><input type='text' name='itemamount[]' value='"+itemprice+"' class='form-control form-control-sm txtamount text-right' disabled></td><td><a href='#' rel='"+itemid+"' class='btn btn-sm btn-remove<?=$divedit?> btn-danger btn-block btn-flat'><i class='fa fa-trash'></i></a></td></tr>");
	...
	$("#itemscontainer<?=$divedit?>").on("keypress keyup keydown",".txtqty",function(){
		...
		$(this).closest('tr').find('.txtamount').val( humanizeNumber(amount) );
		total_now();
	});
