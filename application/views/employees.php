...
$(function () { 
    ...
    $empstatus .= '<option value="In-active" '.($empstat=='In-active'?'selected':'').'>In-active</option>';
    $empstatus .= '<option value="End of Contract" '.($empstat=='End of Contract'?'selected':'').'>End of Contract</option>';
    
    ?>
	
	$('div.dropdownoptions').html('<form id="frmdropdown" method="POST" action="<?=site_url('employees')?>"><select name="empstatus" id="empstatus" class="form-control form-control-sm ml-2"><?=$empstatus?></select></form>');
			  ...
			  <?php if ($this->session->userdata('update_status')): ?>
                ...
                <table id="example2" class="table table-bordered table-hover">
				  ...
				  if($records->num_rows()>0){
                    ...
                    $PNG_WEB_DIR = 'phpqrcode/qrcodes/';    
					$errorCorrectionLevel = 'S';
					$matrixPointSize = 2; 
                    $idcode = $row->idno;
                    $filename = $PNG_WEB_DIR.$idcode.'.png';
                    //if(!file_exists($filename)) QRcode::png($idcode, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
                          
						  ?>
						  <tr>
						  <td><?=$ctr++?></td>
						  <td class='text-center'><img src="<?=$filename?>" style='width:60px;'><br><?=$row->idno?></td>
