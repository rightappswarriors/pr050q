...
$(function(){
    ...
    $('#linktopayable1').click(function() {
        if ($(this).is(':checked')) {
            $('#invoiceor1').removeAttr('disabled');
            $('#invoiceor1').focus();
        } else {
            $('#invoiceor1').attr('disabled', 'disabled');
        }
    });
    
    $("#refno1").on("keyup keypress",function(){
        $("#invoiceor1").val( $(this).val() );
    });
    
	<?php
	$row_arrs="";
	if($stocksin_details->num_rows()>0){
		foreach($stocksin_details->result() as $ind=>$row1){
...
<form action="<?=site_url("stocksin/update_info/").$row->id?>" method="POST" id="frmedit">
 <div class="card-header bg-gradient-info border-0">
  ...
  <div class="col-md-6">
	...
	<div class="form-group">
	  ...
	  <input type="text" placeholder="D.R. No." class="form-control" name="refno" value="<?=$row->refno?>" id="refno1" required>
	  ...
	  <select name="location" class="select2 form-control" style="width:100%" required>
	  <option value="" selected disabled>Select one</option>
		<?php
  ...
  <div class="col-md-6">
	...
	<div class="form-group">
	  ...
	  <input type="date" class="form-control" value="<?=date("Y-m-d",strtotime($row->datereceived))?>" name="datereceived" id="datereceived" required>
	</div>
	<div class="form-group">
	  <label>Remarks</label>
      ...
      <input type="hidden" name="payable" value="<?=$row->payables?>">
      <div class="form-group">
	  <label><input type="checkbox" name="linktopayable" value="1" id="linktopayable1" <?=($row->payables>0?'checked':'')?>> Link to Payables?</label>
      <input type="text" placeholder="Invoice" class="form-control" value="<?=$row->invoiceor?>" name="invoiceor" id="invoiceor1" <?=($row->payables>0?'':'disabled')?>>
	</div>
	...
	<div class="card card-info card-outline">
		...
		<div class="card-body p-0">
			<table class="table table-hover">
				...
				if($stocksin_details->num_rows()>0){
					$tamount = 0;
					foreach($stocksin_details->result() as $ind=>$row){
...
$(function(){
	...
	$("#frmsearchitemedit").submit(function(){
		...
		if(txtitem.length>2){
			...
			setTimeout(function(){ 
				$("#itemsresultedit").load("<?=site_url("items/items_search")?>", {itemsearch:txtitem,divedit:"_edit",nodetails:"<?=$stocksin_details->num_rows()?>"} );
			},1000);
		}
		return false;
	});
