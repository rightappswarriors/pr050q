...
$(function(){ 
	...
	$(".table").on("click",".btnedit",function(){
		...
		$(".cardedit").show(500);
		$(".cardedit").load("<?=site_url("cashadvances/editinfo")?>",{id:rowid});
		return false;
	});
	...
	$(".table").on("click",".btndel",function(){
		...
		$.confirm({
			...
			buttons: {
				confirm: function () {
					window.location.href = "<?=site_url("cashadvances/remove/")?>"+rowid;
				},
				cancel: function () {
					//$.alert('Canceled!');
				}
          ...
          <div class="col-12">
                ...
                <table id="example2" class="table table-bordered table-hover">
				  ...
				  if($records->num_rows()>0){
					  ...
					  foreach($records->result() as $row){
                          ...
                          <td><?=$row->refno?></td>
                          <td><?=$row->employee?></td>
						  <td><?=$this->CI->get_deductiontype($row->deductiontype)?></td>
                          <td class='text-right'><?=number_format($row->deductamount,2)?></td>
						  <td><?=$this->CI->deducton($row->deducton)?></td>
						  <td><?=date("m/d/Y",strtotime($row->transactdate))?>&rarr;<?=date("m/d/Y",strtotime($row->maturedate))?></td>
                          <td><?=$row->purpose?></td>
						  <td class="text-center">
						  <!--<a class="btn btn-success btn-sm btnprint mr-1" target="_blank" href="<?=site_url('cashadvances/print/'.$row->id)?>">
                              <i class="fas fa-print">
                              </i></a>--><a class="btn btn-info btn-sm btnedit <?=$this->session->userdata('pms_editdel')?>" rel="<?=$row->id?>" href="#">
            ...
            <!-- /.card -->
			 <div class="card cardnew card-warning" style="display:none;">
			  ...
			  $this->load->view("cashadvances_addnew",$data)
			  ?>
            </div>
