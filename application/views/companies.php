...
$(function () { 
	...
	$('div.dt_custombutton').html('<a href="<?=site_url("outstocks/addnew")?>" class="btn btnnew btn-warning"><i class="fa fa-plus"></i> New</a>&nbsp;');
	...
	$(".table").on("click",".btnedit",function(){
		...
		$(".cardedit").show(500);
		$(".cardedit").load("<?=site_url("companies/editinfo")?>",{id:rowid});
		return false;
	});
	...
	$(".table").on("click",".btndel",function(){
		...
		$.confirm({
			...
			buttons: {
				confirm: function () {
					window.location.href = "<?=site_url("companies/remove/")?>"+rowid;
				},
				cancel: function () {
					//$.alert('Canceled!');
				}
          ...
          <div class="col-12">
                ...
                <table id="example2" class="table table-bordered table-hover">
				  ...
				  if($records->num_rows()>0){
					  ...
					  foreach($records->result() as $row){
						  ...
						  <tr>
						  <td><?=$ctr++?></td>
						  <td><?=$row->company."<br><small>TIN: ".$row->tin." <span class='text-red'>(".($row->vat=='yes'?'VAT':'NON-VAT').")</span></small>"?></td>
						  <td><?=$row->address?></td>
						  <td><?=$row->contactperson."<br><small>".$row->contact."</small>"?></td>
            ...
            <!-- /.card -->
			 <div class="card cardnew card-warning" style="display:none;">
			  <?php $this->load->view("companies_addnew") ?>
            </div>
