...
$(function () { 
	...
	foreach($invoices->result() as $inv){
		$invoicelist .= '<option value="'.$inv->refno.'">'.addslashes($inv->refno).'</option>';
	}
	endif;
	?>
	
	$('div.dropdownoptions').html('<form id="frmdropdown" method="POST" action="<?=site_url('payables/invoice')?>"><select name="invoice" id="invoice" class="select2 form-control form-control-sm ml-2"><option value="" selected disabled>Show per SI summary</option><?=$invoicelist?></select></form>');
	...
	$(".table").on("click",".btnedit",function(){
		...
		$(".cardedit").show(500);
		//alert(rowid);
		$(".cardedit").load("<?=site_url("adminexpense/editinfo")?>",{id:rowid});
		return false;
	});
	...
	$(".table").on("click",".btndel",function(){
		...
		$.confirm({
			...
			buttons: {
				confirm: function () {
					window.location.href = "<?=site_url("adminexpense/remove/")?>"+rowid;
				},
				cancel: function () {
					//$.alert('Canceled!');
				}
    ...
    $(".table").on("click",".showpayments",function(){
        ...
        setTimeout(function(){
            $("#paymentsresult").load("<?=site_url('adminexpense/showpayments/')?>"+payableid);       
        }, 1000);
    });
          ...
          <div class="col-12">
                ...
                <table id="example2" class="table table-bordered table-hover">
				  ...
				  if($records->num_rows()>0){
					  ...
					  foreach($records->result() as $row){
                          ...
                          if($tpaid>0){
                          ?>
                          <td class='text-right text-bold'><a href='#' class='showpayments' rel='<?=$row->id?>' data-toggle="modal" data-target="#modal-payments"><?=number_format($row->totalamount-$tpaid,2)?></a></td>
                          <?php    
                          }else{
                          ?>
                          <td class='text-right'><?=number_format($row->totalamount-$tpaid,2)?></td>
                          <?php       
                          }    
                          ?>
                          ...
                          if($tpaid>0){
                          ?>      
						  <a class="btn btn-info btn-sm btnedit disabled" rel="<?=$row->id?>" href="#">
				  ...
				  ?>
                    ...
                    <tfoot>
                    <th colspan="5" class='p-2 text-right'>TOTAL</th>
            ...
            <!-- /.card -->
			 <div class="card cardnew card-warning" style="display:none;">
			  <?php 
			  $this->load->view("adminexpense_addnew");
			  ?>
            </div>
