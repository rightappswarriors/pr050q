...
$(function(){
	...
	if($purchaseorder_details->num_rows()>0){
		foreach($purchaseorder_details->result() as $ind=>$row1){
...
<form action="<?=site_url("purchase/update_info/").$row->id?>" method="POST" id="frmedit">
 <div class="card-header bg-gradient-info border-0">
  ...
  <div class="col-md-6">
	...
	<div class="form-group">
	  ...
	  <input type="hidden" placeholder="PO No." class="form-control" name="refno" value="<?=$row->pono?>" id="refno"><input type="text" placeholder="PO No." class="form-control" name="refno1" value="<?=$row->pono?>" id="refno1" disabled>
	</div>
      ...
      <div class="col-md-6">
          <div class="form-group">
	  <label>DR No.</label>
	  <input type="text" placeholder="DR No." class="form-control" value="<?=$row->drno?>" name="drno" id="drno">
	</div></div>
          <div class="col-md-6">
          <div class="form-group">
	  <label>RS No.</label>
	  <input type="text" placeholder="RS No." class="form-control" value="<?=$row->rsno?>" name="rsno" id="rsno">
	</div>
          </div>
      </div>
  ...
  <div class="col-md-6">
	...
	<div class="form-group">
	  <label>Date</label>
	  <input type="date" class="form-control" value="<?=date("Y-m-d",strtotime($row->podate))?>" name="datereceived" id="datereceived" required>
	</div>
	<div class="form-group">
	  <label>Remarks</label>
      ...
      <input type="text" placeholder="Deliver to..." class="form-control" value="<?=$row->addnote?>" name="addnote" id="addnote" value="">
	</div>
		...
		<div class="card-body p-0">
			<table class="table table-hover">
				...
				<tr>
					...
					<td colspan='2' class='text-right total_amount_edit text-bold'><?=number_format($tamount,2)?></td><td>&nbsp;</td>
				</tr>
				</tbody>
			</table>
		</div>
...
$(function(){
	...
	$("#frmsearchitemedit").submit(function(){
		...
		if(txtitem.length>2){
			...
			setTimeout(function(){ 
				$("#itemsresultedit").load("<?=site_url("items/items_search")?>", {itemsearch:txtitem,divedit:"_edit",nodetails:"<?=$purchaseorder_details->num_rows()?>"} );
			},1000);
		}
		return false;
	});
