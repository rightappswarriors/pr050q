...
$(function(){ 
	...
	$(".table").on("click",".btnedit",function(){
		...
		$(".cardedit").show(500);
		$(".cardedit").load("<?=site_url("holidays/editinfo")?>",{id:rowid});
		return false;
	});
	...
	$(".table").on("click",".btndel",function(){
		...
		$.confirm({
			...
			buttons: {
				confirm: function () {
					window.location.href = "<?=site_url("holidays/remove/")?>"+rowid;
				},
				cancel: function () {
					//$.alert('Canceled!');
				}
          ...
          <div class="col-12">
                ...
                <table id="example2" class="table table-bordered table-hover">
				  ...
				  if($records->num_rows()>0){
					  ...
					  foreach($records->result() as $row){
						  ...
						  <tr>
						  <td><?=$ctr++?></td>
                          <td><?=date("m/d/Y",strtotime($row->holidaydate))?></td>
            ...
            <!-- /.card -->
			 <div class="card cardnew card-warning" style="display:none;">
			  <?php 
			  $this->load->view("holidays_addnew");
			  ?>
            </div>
