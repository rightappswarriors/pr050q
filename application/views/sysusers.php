...
$(function(){ 
	...
	$(".table").on("click",".btnedit",function(){
		...
		$(".cardedit").show(500);
		$(".cardedit").load("<?=site_url("sysusers/editinfo")?>",{id:rowid});
		return false;
	});
	...
	$(".table").on("click",".btndel",function(){
		...
		$.confirm({
			...
			buttons: {
				confirm: function () {
					window.location.href = "<?=site_url("sysusers/remove/")?>"+rowid;
				},
				cancel: function () {
					//$.alert('Canceled!');
				}
          ...
          <div class="col-12">
                ...
                <table id="example2" class="table table-bordered table-hover">
				  ...
				  if($records->num_rows()>0){
					  ...
					  foreach($records->result() as $row){
						  ...
						  <td><?=$row->username?></td>
						  <td><?=$this->CI->sysusertype($row->usertype)?></td>
						  <td class="text-right">
						  <?php if($row->usertype>0): ?>
						  <a class="btn btn-info btn-sm btnedit" rel="<?=$row->id?>" href="#">
                              <i class="fas fa-pencil-alt">
                              </i></a>
						<?php if($this->session->userdata('pms_userid')!=$row->id): ?>
                          <a class="btn btn-danger btn-sm btndel" rel="<?=$row->id?>" href="#">
            ...
            <!-- /.card -->
			 <div class="card cardnew card-warning" style="display:none;">
			  <?php $this->load->view("sysusers_addnew"); ?>
            </div>
