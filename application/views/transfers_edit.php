...
$(function(){
	...
	if($transfer_details->num_rows()>0){
		foreach($transfer_details->result() as $ind=>$row1){
...
<form action="<?=site_url("transfers/update_info/").$row->id?>" method="POST" id="frmedit">
 <div class="card-header bg-gradient-info border-0">
	...
	<div class="form-group">
	  ...
	  <option value="" selected disabled>Select one</option>
		...
		foreach($locations->result() as $loc){
			echo "<option value='".$loc->id."' ".(($loc->id==$row->fromlocation)?"selected":"").">".$loc->location."</option>";
		}
		endif;
		?>
	  </select>
	  ...
	  <option value="" selected disabled>Select one</option>
		...
		foreach($locations->result() as $loc1){
			echo "<option value='".$loc1->id."' ".(($loc1->id==$row->tolocation)?"selected":"").">".$loc1->location."</option>";
		}
		endif;
		?>
	  </select>
  ...
  <div class="col-md-6">
	...
	<div class="form-group">
	  <label>Date</label>
	  <input type="date" class="form-control" value="<?=date("Y-m-d",strtotime($row->transferdate))?>" name="transferdate" id="transferdate" required>
	</div>
	<div class="form-group">
	  <label>Remarks</label>
	...
	<div class="card card-info card-outline">
		...
		<div class="card-body p-0">
            <table class="table table-hover">
                ...
                $tamount = 0;
				if($transfer_details->num_rows()>0){
					foreach($transfer_details->result() as $ind=>$row){
...
$(function(){
	...
	$("#frmsearchitemedit").submit(function(){
		...
		if(txtitem.length>2){
			...
			setTimeout(function(){ 
				$("#itemsresultedit").load("<?=site_url("items/items_search_out")?>", {txtlocation:txtlocation,itemsearch:txtitem,divedit:"_edit",nodetails:"<?=$transfer_details->num_rows()?>"} );
			},1000);
		}
		return false;
	});
